...
data "terraform_remote_state" "public_subnet" { // This is to use Outputs from Remote State
  backend = "s3"
  config = {
    bucket = "acs730-week5-jalpatel"            // Bucket from where to GET Terraform State
    key    = "aws-networking/terraform.tfstate" // Object name in the bucket to GET Terraform State
